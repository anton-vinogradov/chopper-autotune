...
Верхний ряд кнопок на сенсорной панели — *и есть* этот план; каждый шаг — одна кнопка (или одна команда в консоли). Сначала механика, потом моторы, потом проверка пределов, а звон от ускорений — штатному инструменту Klipper:
...
3. *Рекомендуется, консоль:* **Ток** (`CHOPPER_CURRENT SAVE=1`, затем снова `CHOPPER_TUNE SAVE=1`) — затюненный чоппер покупает запас момента; потратьте его на меньший и более холодный `run_current`, и перетюньте — оптимум зависит от тока. На референсном стенде это был самый большой выигрыш (моторы в 3.2× холоднее).
4. **Extruder** (`CHOPPER_EXTRUDER SAVE=1`) — третий мотор: греет хотенд (филамент остаётся), находит резонанс E, тюнит, сохраняет.
5. **Envelope** (`CHOPPER_ENVELOPE`) — проверка запаса по скорости/ускорению судьёй-концевиком: на здоровом затюненном принтере вердикт — «мотор не лимит», а практические потолки это поток хотэнда (скорость) и звон (ускорение). Опционально `CHOPPER_MAP PRINT_SPEED=<ваша>` — не сидит ли крейсерская скорость на резонансе (VFA) и где тише рядом.
...
7. **Show** (`CHOPPER_DEMO`) — до/после на слух в любой момент; **Save** сохраняет всё, чего добились последние тюны, одним рестартом.
