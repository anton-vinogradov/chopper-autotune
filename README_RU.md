...
- **Ради чего вообще тюнить spreadCycle.** Меньше вибрации (замеренной на голове), холоднее моторы, чуть больше запаса момента — тот резерв, который универсальные заводские дефолты оставляют на столе. Цель — *вибрация*, а не воспринимаемая громкость (см. [оговорку](#оценка-по-даташитам-а-не-только-по-измерениям)).
...
измерение на железе дополняют друг друга — и ведёт открытые расследования,
сейчас это кейс *щелчков на затюненном конфиге*: измеренные факты, построенные
модели, отброшенные ими гипотезы и следующие шаги. Обновляется по мере
исследования.
...
Каждое действие спрашивает подтверждение перед движением принтера. Во время задачи панель показывает живой прогресс, в простое — по каждому мотору **default → tuned** и насколько **меньше вибрирует** мотор (по последнему Show). Кнопки дёргают те же макросы `CHOPPER_*`, так что с экрана доступно всё то же, что из консоли.
