...
- **Tune A** / **Tune B** — затюнить один мотор (A = `stepper_x`, B = `stepper_y`; chopper — свойство мотора, поэтому это одинаково при любой кинематике, а на CoreXY эти два степпера буквально и есть моторы A и B);
- **Tune both** — затюнить оба мотора за один прогон, вторую сея победителем первой;
- **Save** — вписать последний результат тюнинга каждого мотора в конфиг (сначала бэкап, один рестарт);
- **Show** — поставить дефолты, затем затюненные регистры на **оба** мотора и сделать координированные ходы (оба крутятся вместе, как при печати), чтобы *услышать*, как меняется весь принтер; печатает суммарное падение вибрации;
- **Stop** — прервать задачу; перед выходом тул возвращает регистры и делает re-home.

Каждое действие спрашивает подтверждение перед движением принтера. Во время задачи панель показывает живой прогресс, в простое — по каждому мотору **default → tuned** и насколько **меньше вибрирует** мотор (по последнему Show). Кнопки дёргают те же макросы `CHOPPER_*`, так что с экрана доступно всё то же, что из консоли. (`CHOPPER_CURRENT` пока только из консоли.)
...
**CHOPPER_SAVE** — вписать последний результат тюнинга каждого мотора в конфиг за один батч-рестарт (с бэкапом); логирует, какого победителя и из какого датасета сохраняет. Сохранить достигнутое последним тюнингом — тюнил ли ты моторы по отдельности или вместе.
...
**CHOPPER_STOP** — прервать идущий прогон тюнинга/шоу; тул возвращает регистры, выходит из spreadCycle и делает re-home перед выходом.
...
- [ ] Моторы за пределами `stepper_x`/`stepper_y` (вторая Y, IDEX, экструдер)
