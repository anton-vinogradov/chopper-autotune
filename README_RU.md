...
Если у вас есть [KlipperScreen](https://github.com/KlipperScreen/KlipperScreen), `install.sh` добавляет в его главное меню кнопку **Chopper** (сливается с вашим меню, ничего не переписывая). Один тап открывает панель:

- **Tune X** / **Tune Y** — затюнить одну ось и напечатать рекомендацию;
- **Both + Save** — затюнить обе оси и вписать победителей в конфиг;
...
Каждое действие спрашивает подтверждение перед движением принтера. Во время задачи панель показывает живой прогресс, в простое — регистры, сейчас сохранённые для каждой оси. Кнопки дёргают те же макросы `CHOPPER_*`, так что с экрана доступно всё то же, что из консоли.
...
CHOPPER_DEMO                         ; показать дефолты против затюненного, бок о бок
```
...
**CHOPPER_DEMO** — меряет дефолтные регистры драйвера против сохранённых/затюненных на резонансной скорости (чередуя для честности) и печатает, во сколько раз тише стала ось. `AXIS`, `SPEED` (авто, если опущен), `DEFAULT=tbl,toff,hstrt,hend` (по умолчанию `2,3,5,0`), `ITERATIONS`.
