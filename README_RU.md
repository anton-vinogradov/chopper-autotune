...
- **Show** — проиграть дефолты драйвера против затюненных регистров на обоих моторах (сначала A, потом B), чтобы *услышать* разницу, показывая что играет;
...
**CHOPPER_DEMO** — проигрывает дефолтные регистры драйвера против сохранённых/затюненных на резонансной скорости, чередуя, чтобы *услышать* разницу, объявляя каждый заход на экране и в консоли. `MOTOR` (a/b/**ab** — по умолчанию оба, один за другим), `SPEED` (авто, если опущен), `ROUNDS`, `REPEATS`. `REPORT=1` печатает замер с числами (насколько меньше вибрация, с барами) вместо звукового шоу; `DEFAULT=tbl,toff,hstrt,hend` (по умолчанию `2,3,5,0`) и `ITERATIONS` — для замера.
