...
Каждое действие спрашивает подтверждение перед движением принтера. Во время задачи панель показывает живой прогресс, в простое — по каждому мотору **default → tuned** и насколько **тише** стал мотор (по последнему Show). Кнопки дёргают те же макросы `CHOPPER_*`, так что с экрана доступно всё то же, что из консоли.
