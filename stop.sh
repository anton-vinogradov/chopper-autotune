#!/bin/bash
# Stop a running chopper-autotune motion job (tune/collect/find-speed/map/demo/current/envelope).
...
if pkill -TERM -f "[c]hopper-autotune (tune|collect|find-speed|map|demo|current|envelope)"; then
