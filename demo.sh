...
log=~/printer_data/config/chopper-autotune/demo.log
setsid "$here/.venv/bin/chopper-autotune" demo "$@" > "$log" 2>&1 < /dev/null &
echo "chopper-autotune demo started (PID $!); progress on the display/console, log: $log"
