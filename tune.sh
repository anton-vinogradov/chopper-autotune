...
log=~/printer_data/config/chopper-autotune/tune.log
setsid "$here/.venv/bin/chopper-autotune" tune "$@" > "$log" 2>&1 < /dev/null &
echo "chopper-autotune tune started (PID $!); progress on the display/console, log: $log"
