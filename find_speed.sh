...
log=~/printer_data/config/chopper-autotune/find_speed.log
setsid "$here/.venv/bin/chopper-autotune" find-speed --yes "$@" > "$log" 2>&1 < /dev/null &
echo "chopper-autotune find-speed started (PID $!); progress on the display/console, log: $log"
