...
- **Belts** — match the two CoreXY belts: measures both and shows which to tighten, the gap, and how it changed since last time, then releases the motors so you can reach in;
...
**CHOPPER_BELTS** — **CoreXY belt-tension match**. The two belts should resonate at the same frequency (resonance goes as √tension), so a lower peak means a looser belt. Excites each belt on its own by driving one motor's diagonal (A = head `1,1`, B = head `1,-1`) with Klipper's swept-sine `TEST_RESONANCES`, then computes the response spectrum here (numpy in the tool's venv — nothing needed in klippy-env) and prints a verdict: *balanced*, or *belt X is looser, tighten it* with the tension gap and how it changed since the previous run. On a mismatch it releases the gantry motors so the belt is easy to reach. `SHOW=A`/`SHOW=B` skips the measurement and just jogs that belt — on CoreXY that diagonal moves only that belt's loop — so you can see which one is which (also the **Motor A**/**Motor B** touchscreen buttons). Needs a configured `[resonance_tester]` (same as input-shaper calibration); CoreXY/H-bot only. Read-only. Parameters: `MIN_FREQ`, `MAX_FREQ`, `HZ_PER_SEC`, `TOLERANCE`, `SHOW`, `DRY_RUN`.
...
- [x] Belt-tension match (`CHOPPER_BELTS`): resonance-compare the two CoreXY belts via swept-sine and say which to tighten (PSD computed in the tool's own venv) — accelerometer-native, complements the per-motor chopper work
