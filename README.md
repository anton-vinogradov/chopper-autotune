...
- Z keeps its homing: the tools switch off only the X/Y motors, never `M18` or `M84` (except after a thermal stop). A printer whose `G28` lifts an unhomed Z (`[safe_z_home] z_hop`, RatOS `[ratos_homing] z_hop`, Beacon `home_z_hop`) would lift it again on every X/Y homing, and a run re-homes X/Y many times. So on such a printer a job refuses to start with Z unhomed, like Klipper's own tools: clear the bed, run `G28`, start again;
