...
- Klipper v0.13 or later + Moonraker (Mainsail, Fluidd or any other frontend). [Kalico](https://github.com/KalicoCrew/kalico) since July 2026 works too, including `[resonance_tester] accel_chips` and the `limited_corexy` kinematics. On an older Klipper or Kalico the tools refuse to start: they would measure the standstill after each move there, or mark Z homed. `CHOPPER_ENVELOPE` does not support Kalico's `limited_*` kinematics yet: they cap each belt by `max_x_accel`/`max_y_accel`, whatever acceleration the test asks for.
