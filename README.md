...
- driver temperature is watched: before a run starts and before every move (every stroke pair in `CHOPPER_CURRENT` / `CHOPPER_ENVELOPE`) the tools read the X/Y drivers' over-temperature flags and the TMC2240 die temperature (stop at 110 °C). At the first warning they stop with the motors off (`M18`) and no re-home, aiming to stop before the driver shuts itself down. A TMC2240 left at Klipper's default `slope_control` 0 without `klipper_tmc_autotune` gets a note: autotune sets 3.
