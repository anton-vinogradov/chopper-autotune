...
- Z keeps its homing: the tools switch off only the X/Y motors, never `M18` (except after a thermal stop). With `[safe_z_home] z_hop`, Klipper lifts an unhomed Z by `z_hop` on every `G28 X Y`, and a run re-homes X/Y many times, so a job that starts with Z unhomed on such a printer homes all axes once first;
