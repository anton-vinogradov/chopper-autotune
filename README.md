...
- **Belts** — compare the two CoreXY belt diagonals: the response gap and how it changed since last time, then releases the motors so you can reach in (a gap can be tension or structure — see the command reference);
...
**CHOPPER_BELTS** — **CoreXY belt-diagonal response comparison**. Excites each belt's diagonal on its own (A = head `1,1`, B = head `1,-1`) with Klipper's swept-sine `TEST_RESONANCES`, computes the response spectrum here (numpy in the tool's venv — nothing needed in klippy-env), and reports the two response frequencies, the gap, and the per-belt change since the previous run. **Honest scope (measured):** on a symmetric CoreXY matched belts give matched responses, but a persistent gap can be tension mismatch *or structural asymmetry* — on the reference rig a heavy overtension moved the response by 0 Hz, so the tool never orders "tighten belt X"; it tells you which diagonal responds lower, and if nothing moved since the last run it says the response does not track tension on this machine. On a gap it releases the gantry motors so the belts are easy to reach. `SHOW=A`/`SHOW=B` skips the measurement and just jogs that belt — on CoreXY that diagonal moves only that belt's loop — so you can see which one is which (also the **Motor A**/**Motor B** touchscreen buttons). Needs a configured `[resonance_tester]`; CoreXY/H-bot only. Read-only. Parameters: `MIN_FREQ`, `MAX_FREQ`, `HZ_PER_SEC`, `TOLERANCE`, `SHOW`, `DRY_RUN`.

**CHOPPER_BELTS PLUCK=1** — the **tension measurement proper**: you pluck belt spans like guitar strings on the display's cue, and the toolhead accelerometer listens. The pluck excites the **transverse string mode** — the one that *is* tension (f ∝ √T) — and the tool identifies the fundamental by its **(f, 2f) pair**: the ringing span shakes its anchor laterally at f and axially at 2f (the string's tension pulses twice per cycle), so a lone unpaired line is flagged as a possible harmonic of a weak pluck (measured: a 4f line masquerading at 400 Hz). The protocol has a **built-in structural control**: for each belt you pluck the span **left of the head, then right of it** — same loop, same tension, equal lengths with the head parked at center, so the two must agree (within 3 %) before belts are compared; a disagreement flags the reading instead of trusting it. Verdict: tension ratio (f²) between belts; with `SPAN=` (plucked span length, cm) also **absolute newtons**, T = μ·(2·L·f)²; `MU=` overrides the belt's linear density (default 7.7 g/m, GT2 6 mm). Also the **Pluck** touchscreen button. Parameters: `PLUCK`, `SPAN`, `MU`, `PLUCKS`, `TOLERANCE`, `DRY_RUN`.
