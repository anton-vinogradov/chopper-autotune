...
- **Results** — an on-demand summary of everything measured so far: default → tuned registers per motor (A/B/E) with the vibration drop, run currents, the last belt-tension plucks, and the achieved envelope ceilings (speed/acceleration; `350+` = held the whole tested range);
