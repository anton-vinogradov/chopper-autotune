...
- Z keeps its homing: the tools switch off the gantry and head motors (X/Y, extruders), never `M18` or `M84` during a run. A thermal stop switches X/Y off and forgets their homing while Z keeps holding; where Klipper cannot forget the X/Y homing alone (before v0.13, Kalico before July 2026), an `M84` does it, so Z loses its homing too and goes straight back on. A printer whose `G28` lifts an unhomed Z (`[safe_z_home] z_hop`, RatOS `[ratos_homing] z_hop`, Beacon `home_z_hop`) would lift it again on every X/Y homing, and a run re-homes X/Y many times. So on such a printer a job refuses to start with Z unhomed, like Klipper's own tools: clear the bed, run `G28`, start again. Every `SAVE=1` restarts Klipper, which forgets all homing: there, home all axes before the next step. Not covered: `[homing_override]` with `set_position_z` (the stock Voron 0.2) resets and moves Z on every X/Y homing; home all axes after a run and watch the Z travel;
- driver temperature is watched: before a run starts and before every move (every stroke pair in `CHOPPER_CURRENT` / `CHOPPER_ENVELOPE`, every sweep in `CHOPPER_BELTS SWEEP=1`) the tools read the X/Y drivers' over-temperature flags and the TMC2240 die temperature (stop at 100 °C: in [#133](https://github.com/anton-vinogradov/chopper-autotune/issues/133) drivers shut down at 112–121 °C, five of six with no over-temperature warning first). Waits at standstill with the motors under current, like the pluck windows of `CHOPPER_BELTS`, are checked at least once a second. At the first warning the tools stop, switch the X/Y motors off and forget their homing, with no re-home, aiming to stop before the driver shuts itself down. Z keeps holding. A die temperature the sensor cannot report (below −40 °C or above 165 °C) counts as no reading, with a warning. When Klipper shuts down anyway, a run stops at once instead of retrying every move. A TMC2240 left at Klipper's default `slope_control` 0 without `klipper_tmc_autotune`, or holding its full run current at standstill (no `hold_current` below it), gets a note.
...
- You can keep autotune. To use tuned registers on that motor instead:
  1. In its `[tmc…]` section, set what autotune sets there, replacing any such line already present. That is the StallGuard threshold: `driver_SGTHRS` on a TMC2209 (autotune's `sg4_thrs`), `driver_SGT` on a TMC2130, TMC2240, TMC2660 or TMC5160, and on a TMC2240 also `driver_SG4_THRS` (autotune's `sg4_thrs`, 0 included) and `driver_SLOPE_CONTROL: 3`, which keeps the driver cooler. A TMC2208 has no StallGuard.
  2. Remove the `[autotune_tmc …]` section and restart Klipper.
  3. If the motor homes sensorless, re-tune the StallGuard threshold it homes on before anything else ([Klipper's guide](https://www.klipper3d.org/TMC_Drivers.html#sensorless-homing); on a TMC2240 homing on SG4 that is `driver_SG4_THRS`). The value comes from the autotune section, or its default, and ran under autotune's CoolStep and PWM, which go with the section, so the carried value is only a starting point.
  4. Tune again. A run under autotune measured with its CoolStep on, which lowers the current under load, and the chopper optimum depends on the current. Such results are not saved, not even after autotune is gone, and a dataset is not resumed across the change. A TMC2208 has no CoolStep, so its results are not held back.
- At the end of a run the tool puts back the registers and the mode the driver ran before it. On autotune's motors it reads them live from the driver, and the live mode wins over a `stealthchop_threshold` in the config, since autotune sets the mode itself. If the register read fails, it puts back the config registers, and autotune's return at the next Klipper restart. A run stopped before that read has written nothing and puts nothing back there.
...
```
G28                     ; home all axes first (each SAVE=1 restarts Klipper: home again after it)
...
- **Restore** — the undo: choose **Klipper defaults** (stock chopper registers, tuning off; `run_current` stays — it is a measured safety setting) or **Backup** (the config exactly as before the last save, currents included). Either way the plan's step marks and the measured Results reset — thresholds, ceilings and the map were measured against the registers just rolled away (the extruder winner memory survives for `SAVE_LAST=1`);
...
Every action confirms before it moves (or heats) the printer. While a job runs the panel shows live progress and finishes with a popup verdict; when idle it shows, per motor, the **default → tuned** registers and how much **less it vibrates** (measured by the last Show). The buttons drive the same `CHOPPER_*` macros, so anything you can do from the console you can do from the screen. (Every macro is on the panel now; the console keeps the parameterised forms.)
...
**CHOPPER_RESTORE** — the undo. `DEFAULTS=1` writes each driver's own stock chopper registers (`2/3/5/0` on TMC2208/2209, `2/3/5/2` + TPFD 4 on TMC2240/5160, `1/4/0/7` on TMC2130, `2/4/3/3` on TMC2660) into every tuned TMC section — tuning off, `run_current` untouched (it is a measured safety setting). `BACKUP=1` puts back the `*.chopper-backup.cfg` snapshots taken before the last save — registers AND currents. One restart either way; refuses while printing.

**CHOPPER_CURRENT** — find the minimal safe `run_current` per motor: a worst-case single-motor stress pattern (full machine accel, belt speeds through 200 mm/s) with an **endstop referee** — skipped steps land as a position offset that an endstop creep measures deterministically, because a stall can be nearly silent (measured; see [docs/SCIENCE.md](docs/SCIENCE.md)). Bisects to the skip threshold and recommends `threshold × MARGIN` (default `2.0`); `SAVE=1` writes `run_current` (backup first) and restarts. Re-run `CHOPPER_TUNE` afterwards — the chopper optimum depends on the current. On coupled-XY both motors are written the **max** of the two recommendations, though never above each motor's own configured `run_current` (the config is each motor's rating boundary — A/B are not guaranteed identical parts): they share one kinematics and the threshold gap is mechanical drag that drifts (measured: a per-motor 0.60 A hit a speed ceiling its 0.95 A twin did not). `PER_MOTOR=1` keeps the measured split. Parameters: `MOTOR`, `MARGIN`, `MIN_CURRENT`, `RESOLUTION`, `ACCEL`, `SAVE`, `DRY_RUN`.
...
- **An accelerometer on the toolhead** — the measuring instrument of the whole tool:
  - these chips work: ADXL345 (the classic), LIS2DW, LIS3DH, the MPU-9250 family, ICM-20948, BMI160 (Klipper master only), and the accelerometer built into a Beacon probe (RevH, set `accel_chip: beacon` in `[resonance_tester]`). USB sticks (KUSBA, FYSETC PIS) and CAN toolhead boards with an onboard chip (EBB36/42, SB2209, …) count too;
